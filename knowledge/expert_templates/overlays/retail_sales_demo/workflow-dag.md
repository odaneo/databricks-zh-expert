...
本资产为 `retail_sales_demo` 模拟项目定义 Lakeflow Jobs 与 Pipeline 的依赖草案，扩展通用工作流蓝图。调度时间和重试策略需要结合目标 workspace、数据量和支持时段重新确认。
...
4. 核心任务的 99.5% 是模拟月度成功率目标；统计口径排除项和维护窗口必须由项目确认。
