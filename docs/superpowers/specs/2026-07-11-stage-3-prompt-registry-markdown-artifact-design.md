...
| `sql_generation` | Databricks SQL | `sql` | `1.0.0` | 是 |
| `pyspark_generation` | PySpark | `pyspark` | `1.0.0` | 是 |
...
5. 其余五类属于文档型 Artifact，第一块内容必须是且只能有一个一级标题 `# 标题`。
...
`pyspark` 只要求直接输出 `python` fenced code block。输入、输出、参数和关键假设使用简短 `#` 注释。
不强制模型重复输出“使用场景”“前置条件”“参数说明”“注意事项”等章节。确有无法放入注释的重要
风险时，可以在代码块后补充最多三条简短列表项，但不得先写长篇解释。
...
9. 文档型 Artifact 的缺失 H1、重复 H1、章节缺失和顺序错误均被拒绝。
10. 代码型 Artifact 不要求 H1；代码围栏不是第一块、语言错误或缺失时被拒绝。
