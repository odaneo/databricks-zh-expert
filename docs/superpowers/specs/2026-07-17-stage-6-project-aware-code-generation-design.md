...
7. 在 Session、`model_calls` 和 Trace 1.6 中审计 Workspace、选中来源和提案状态。
...
| `workspace_mode` | 固定为 `greenfield` |
| `id` | 小写英文、数字和下划线，全 Registry 唯一 |
| `display_name` | 中文显示名称 |
| `description` | 中文说明 |
| `version` | 严格 `MAJOR.MINOR.PATCH` |
| `cloud` | 阶段 6 固定为 `aws` |
| `is_mock` | 内置 Demo 为 `true`；真实项目为 `false` |
...
Workspace API 返回 ID、名称、说明、版本、`workspace_mode=greenfield`、云、Mock 标记、Source 数、Hash 和相对路径，
不返回正文或绝对路径。
...
4. `model_calls.workspace_mode`。
5. `model_calls.workspace_source_hash`。
6. `model_calls.workspace_context`：实际选中的输入单元。
7. Artifact 类型和 `project_fact_status=proposal`。
