...
4. API 只接受 `gpt5.5`、`gpt5.4mini`、`deepseek-v4-flash`、`deepseek-v4-pro` 四个业务别名，不接受 LiteLLM ID。
...
- 产出：`ModelAlias = Literal["gpt5.5", "gpt5.4mini", "deepseek-v4-flash", "deepseek-v4-pro"]`。
- 产出：`ModelProvider = Literal["openai", "deepseek"]`。
...
- [ ] **步骤 1：先写固定白名单和注册表失败测试**

在 `tests/unit/test_model_registry.py` 写入以下测试形状，四个别名的顺序也作为 API 稳定契约：

```python
from databricks_zh_expert.llm.model_registry import MODEL_ALIASES, ModelRegistry
...
def test_registry_exposes_only_the_fixed_model_aliases(settings_factory) -> None:
    ...
    assert registry.get("gpt5.5").litellm_model == "openai/gpt-5.5"
    assert registry.get("deepseek-v4-flash").provider == "deepseek"
...
def test_registry_reports_provider_key_configuration(settings_factory) -> None:
    ...
    assert registry.get("gpt5.4mini").configured is True
    assert registry.get("deepseek-v4-pro").configured is False
```

- [ ] **步骤 2：补充 Settings 的失败测试矩阵**
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"model_gpt55_id": "deepseek/gpt-5.5"}, "必须使用 openai/ 前缀"),
        ({"model_deepseek_v4_pro_id": "openai/deepseek-v4-pro"}, "必须使用 deepseek/ 前缀"),
    ],
)
...
同时把 `FALLBACK_MODELS`、`DEFAULT_TEMPERATURE` 和四个 `MODEL_*_ID` 加入必填字段测试和环境变量清理列表。
...
预期：`model_registry` 尚不存在，且 `Settings` 尚无阶段 2 字段，测试失败；不得出现真实模型网络请求。
...
class ModelRegistry:
    ...
    def from_settings(cls, settings: "Settings") -> "ModelRegistry":
        ...
        return cls(
            ...
            models=(
                ModelDefinition("gpt5.5", "GPT-5.5", "openai", settings.model_gpt55_id, openai_configured),
                ModelDefinition("gpt5.4mini", "GPT-5.4 mini", "openai", settings.model_gpt54mini_id, openai_configured),
                ModelDefinition("deepseek-v4-flash", "DeepSeek V4 Flash", "deepseek", settings.model_deepseek_v4_flash_id, deepseek_configured),
                ModelDefinition("deepseek-v4-pro", "DeepSeek V4 Pro", "deepseek", settings.model_deepseek_v4_pro_id, deepseek_configured),
            ),
        )
...
实现时按 Ruff 的 100 字符行宽拆分四个 `ModelDefinition` 调用，不改变字段和值。

- [ ] **步骤 5：实现 Settings 的 CSV 解析和启动校验**
...
class Settings(BaseSettings):
    ...
    model_gpt55_id: str = Field(min_length=1)
    model_gpt54mini_id: str = Field(min_length=1)
    model_deepseek_v4_flash_id: str = Field(min_length=1)
    model_deepseek_v4_pro_id: str = Field(min_length=1)
    ...
    def validate_model_configuration(self) -> Self:
        ...
        model_ids = (
            ("MODEL_GPT55_ID", "openai", self.model_gpt55_id),
            ("MODEL_GPT54MINI_ID", "openai", self.model_gpt54mini_id),
            ("MODEL_DEEPSEEK_V4_FLASH_ID", "deepseek", self.model_deepseek_v4_flash_id),
            ("MODEL_DEEPSEEK_V4_PRO_ID", "deepseek", self.model_deepseek_v4_pro_id),
        )
        for field_name, provider, model_id in model_ids:
            prefix, separator, name = model_id.partition("/")
            if prefix != provider or separator != "/" or not name:
                raise ValueError(f"{field_name} 必须使用 {provider}/ 前缀并包含模型名。")
        return self
...
- [ ] **步骤 6：保持阶段 1 LiteLLM 客户端在配置迁移期间可用**
...
default_model: ModelAlias = "deepseek-v4-flash"
fallback_models: str | tuple[ModelAlias, ...] = "deepseek-v4-flash,gpt5.4mini"
default_temperature: float = 0.2
model_gpt55_id: str = "openai/gpt-5.5"
model_gpt54mini_id: str = "openai/gpt-5.4-mini"
model_deepseek_v4_flash_id: str = "deepseek/deepseek-v4-flash"
model_deepseek_v4_pro_id: str = "deepseek/deepseek-v4-pro"
```
...
在 `.env` 中补入 Task 1 的 `FALLBACK_MODELS`、`DEFAULT_TEMPERATURE` 和四个 `MODEL_*_ID`，把旧的 provider-qualified `DEFAULT_MODEL` 改为业务别名 `deepseek-v4-flash`。保留现有真实 API Key，不提交 `.env`。
...
```markdown
`DEFAULT_MODEL` 和 `FALLBACK_MODELS` 使用 `.env.example` 中的业务别名；四个 `MODEL_*_ID` 保存对应的 LiteLLM 模型 ID。
```
