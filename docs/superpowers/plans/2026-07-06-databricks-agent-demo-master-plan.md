...
**架构：** 采用本地单体后端和 API 调试优先的结构。阶段 1 至阶段 9 已完成模型调用、会话保存、Prompt、RAG、专家模板、Workspace 上下文、Markdown 交付物和固定评估；后续先重建更真实的 Workspace 基准，再增加只读文件检索与工具系统，最后用 LangGraph 编排复杂顾问流程。Agent 不直接操作 Databricks，不执行 SQL，不提交 Job，只负责生成建议、代码草稿、设计文档和引用来源。Web UI 与桌面客户端后置。
...
受控本地文件检索、LangGraph（阶段 12）、pytest、Ruff、Pyright、Markdown。
...
4. 多用户账号系统。
5. 企业级权限、审计、租户隔离。
6. 基础 Demo 不包含完整 Web UI 或桌面客户端，二者在后续阶段实现。
...
价值。Markdown 预览和另存为 `.md` 合并到阶段 13 Web UI。
...
4. 新基线作为阶段 11 文件检索和阶段 12 LangGraph 的共同控制组。
...
暂缓。只有在阶段 10 至阶段 12 的 Workspace、文件工具和编排 API 稳定后才编写详细计划。

### 候选范围

1. 聊天、会话历史和模型选择。
2. Workspace 状态、文件证据和来源预览。
3. Markdown Artifact 预览与下载。
4. LangGraph 任务状态、待确认事项和失败信息。
5. 知识库与专家模板索引状态。

---

## 阶段 14：桌面客户端与本地工作区（后置）

### 状态

暂缓。最终形态允许用户选择本地项目文件夹，但复用阶段 11 的只读工具契约和阶段 12 的编排接口，不在客户端
重新实现检索逻辑。

### 候选范围

1. C# Avalonia 或 Electron，技术选型在阶段 specs 中决定。
2. 本地文件夹选择、授权根目录和最近 Workspace。
3. 增量扫描、文件 Hash、忽略规则和变更检测。
4. SQLite 保存可重建的解析结果、Chunk、FTS5 索引和客户端设置。
5. 项目源码始终保留在用户目录，不把完整项目复制到 PostgreSQL。
...
6. 阶段 6：代码生成模块。
7. 阶段 7：工作流设计模块。
8. 阶段 8：跳过独立实施，Markdown 预览与下载合并到阶段 13。
9. 阶段 9：固定评估集。
10. 阶段 10：真实 Northwind Workspace 与评估再基线化。
11. 阶段 11：Workspace 只读文件检索与工具系统。
12. 阶段 12：LangGraph 顾问工作流编排。
13. 阶段 13：简单 Web UI，后置。
14. 阶段 14：桌面客户端与本地工作区，后置。

---

## 6. 后续需要进一步细化的小计划

阶段 1 至阶段 9 已有计划或明确跳过。后续按以下顺序分别编写 specs 和小计划，不把三个独立子系统塞入同一份
实施计划：

1. 阶段 10：真实 Northwind Workspace 与评估再基线化。
2. 阶段 11：Workspace 只读文件检索与工具系统。
3. 阶段 12：LangGraph 顾问工作流编排。
4. 阶段 13：简单 Web UI，待阶段 12 API 稳定后再写。
5. 阶段 14：桌面客户端与本地工作区，待 Web UI 验证交互后再写。
...
当前取舍：阶段 10 至阶段 12 只强化真实项目理解、只读检索和顾问编排；Web UI 与桌面端继续后置，系统始终不做
Databricks 执行。
...
已经完成。近期下一步是为阶段 10 编写 specs 和逐任务计划，人工校订 Northwind Workspace，更新
`workspace_context.yml` 与 `end_to_end.yml`，随后使用当前 Direct 流程重新运行 Flash、Pro 双模型基线。

阶段 10 完成后再依次编写阶段 11 文件检索和阶段 12 LangGraph 的独立 specs 与计划；Web UI 和桌面客户端暂不进入
近期实施队列。
