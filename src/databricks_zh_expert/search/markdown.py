...
class MarkdownChunker:
    ...
    def split(self, source: MarkdownSource) -> tuple[MarkdownChunk, ...]:
            ...
            for content in self._pack_blocks(prefix, expanded):
                chunks.append(
                    MarkdownChunk(
                        ...
                        source_ref=self._source_ref(source.source_ref, section.anchor),
                    )
                )
        return tuple(chunks)

    def _sections(self, source: MarkdownSource) -> tuple[_Section, ...]:
        ...
        for index, token in enumerate(tokens):
            ...
            anchor = (
                source.heading_anchors[len(headings)]
                if len(headings) < len(source.heading_anchors)
                else None
            )
            if anchor is None:
    ...
    @staticmethod
    def _source_ref(source_ref: str, anchor: str | None) -> str:
        return source_ref if anchor is None else f"{source_ref}#{anchor}"
